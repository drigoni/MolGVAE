...
class MLP(object):
    def __init__(self, in_size, out_size, hid_sizes, dropout_keep_prob, activation_function=tf.nn.relu, name=None, init_function=None):
    ...
    def print(self, operation):
        for W, b in zip(self.params["weights"], self.params["biases"]):
            operation = tf.Print(operation, [W, tf.reduce_mean(W)], message=W.name,
                                   summarize=1000)  # TODO: pr
            operation = tf.Print(operation, [b, tf.reduce_mean(b)], message=b.name,
                                   summarize=1000)  # TODO: pr
        return operation
...
class MLP_norm(object):
    ...
    def print(self, operation):
        for W, b in zip(self.params["weights"], self.params["biases"]):
            operation = tf.Print(operation, [W, tf.reduce_mean(W)], message=W.name,
                                   summarize=1000)  # TODO: pr
            operation = tf.Print(operation, [b, tf.reduce_mean(b)], message=W.name,
                                   summarize=1000)  # TODO: pr
        return operation
